  ...
  const fetchArticle = useCallback(async (id: string) => {
    try {
      ...
      if (response.ok && data.success) {
        ...
        setTitle(article.translations?.[0]?.title || "");
        setSlug(article.slug || "");
        setExcerpt(article.translations?.[0]?.excerpt || "");
        setCoverImage(article.coverImage || "");
        setContent(article.translations?.[0]?.content || "");
        ...
        setDownloadFile(article.downloadFile || "");
        setDownloadFileName(article.downloadFileName || "");
        setDownloadFileSize(article.downloadFileSize || null);
        setDownloadLink(article.downloadLink || "");
        setDownloadIsFree(article.downloadIsFree !== false);
        setDownloadPrice(article.downloadPrice?.toString() || "");
      } else {
        alert("获取文章失败");
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center px-5 py-2.5 text-white bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl hover:from-blue-600 hover:to-blue-700 shadow-lg shadow-blue-500/25 transition-all hover:shadow-blue-500/40 hover:-translate-y-0.5 disabled:opacity-50 disabled:shadow-none"
          >
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center px-5 py-2.5 text-white bg-gradient-to-r from-green-500 to-green-600 rounded-xl hover:from-green-600 hover:to-green-700 shadow-lg shadow-green-500/25 transition-all hover:shadow-green-500/40 hover:-translate-y-0.5 disabled:opacity-50 disabled:shadow-none"
          >
            {saving ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Save className="h-4 w-4 mr-2" />}
            保存草稿
          </button>
