...
create_release_structure() {
    ...
    # 如果是完整构建，复制 node_modules
    if [ "$BUILD_TYPE" = "full" ]; then
        log_info "复制 node_modules（完整构建）..."
        cp -r "$PROJECT_ROOT/node_modules" "$RELEASE_DIR/"
        cp -r "$PROJECT_ROOT/apps/web/node_modules" "$RELEASE_DIR/apps/web/" 2>/dev/null || true
        cp -r "$PROJECT_ROOT/packages/database/node_modules" "$RELEASE_DIR/packages/database/" 2>/dev/null || true
    fi
}
