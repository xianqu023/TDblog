...
export default function ComponentRenderer({ component }: ComponentRendererProps) {
  const { removeComponent, updateComponent } = useThemeEditorStore();
    ...
    switch (type) {
        ...
        return (
          ...
          >
            {Array.from({ length: props.columns || 3 }).map((_, i) => (
              <div
                key={i}
                className="bg-gray-100 rounded-lg p-6 text-center text-gray-400 text-sm border-2 border-dashed border-gray-200"
              >
                网格 {i + 1}
              </div>
            ))}
        ...
        return (
          <div className="py-6 px-6 flex gap-4">
            {Array.from({ length: props.columns || 2 }).map((_, i) => (
              <div
                key={i}
                className="flex-1 bg-gray-100 rounded-lg p-6 text-center text-gray-400 text-sm border-2 border-dashed border-gray-200"
      ...
      case "section":
        return (
          ...
          >
            <div className="max-w-4xl mx-auto text-center text-gray-400 text-sm py-8 border-2 border-dashed border-gray-200 rounded">
              内容区块 - 可添加子组件
            </div>
          </div>
        );
              ...
              {Array.from({ length: Math.min(props.limit || 6, 6) }).map((_, i) => (
                <div key={i} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    {props.showExcerpt && (
                      <p className="text-sm text-gray-600 mb-3 line-clamp-2">这是一段文章摘要，展示文章的部分内容...</p>
                    )}
                    {props.showMeta && (
      ...
      case "stats":
        return (
          <div className="py-12 px-6 bg-gradient-to-r from-blue-600 to-purple-600 text-white">
            <div className={`max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-${props.columns || 3} gap-8 text-center`}>
      ...
      case "testimonial":
        return (
          <div className="py-12 px-6">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-${props.columns || 2} gap-6 max-w-4xl mx-auto`}>
                ...
                >
                  ...
                  <button
                    className={`w-full py-2 rounded-lg font-medium transition-colors ${
                      plan.popular ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-100 text-gray-800 hover:bg-gray-200"
                    }`}
                  >
                    选择方案
                  </button>
            ...
            <div className="relative rounded-lg overflow-hidden bg-gray-100" style={{ aspectRatio: props.aspectRatio || "16/9" }}>
              <div className="absolute inset-0 flex items-center justify-center text-gray-400">
                <div className="text-center">
                  ...
                  <p>轮播图组件</p>
                  <p className="text-sm mt-1">{(props.slides || []).length} 张幻灯片</p>
                </div>
              </div>
            </div>
          </div>
      ...
      default:
        return (
          <div className="py-8 px-6 bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg text-center text-gray-500">
            <div className="text-4xl mb-2">🧩</div>
            <p className="font-medium">{type}</p>
            <p className="text-sm mt-1">组件渲染中...</p>
          </div>
        );
    }
  };
